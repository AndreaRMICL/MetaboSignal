...
link_reaction_gene = function(enzyme, gene, ko, reactionTable) {
    ...
    if (length(index) > 0) {
        ...
        line = cbind(reactions, rep(enzyme, length(reactions)),
            rep(gene, length(reactions)), rep(ko, length(reactions)))
...
build_RG_edge = function (edge, reactionko, reactiongene, expand_genes) {
    ...
    if (expand_genes == FALSE) {
      # Genes represent orthology IDs.
      index_reaction = which(reactionko[, 1] == reaction)

      if (length(index_reaction) > 0) {
        reaction_genes = paste(reaction, reactionko[index_reaction, 2], sep = "_")
      } else {
        reaction_genes = reaction
      }
      reaction_genes_all = c(reaction_genes_all, reaction_genes)
    } else {
      # Genes represent organism specific IDs.
      index_reaction = which(reactiongene[, 1] == reaction)

      if (length(index_reaction) > 0) {
        reaction_genes = paste(reaction, reactiongene[index_reaction, 2], sep = "_")
      } else {
        reaction_genes = reaction
      }
      reaction_genes_all = c(reaction_genes_all, reaction_genes)
    ...
    if (index_rn == 1) {
      new_edges = cbind(reaction_genes_all, rep(metabolite, length(reaction_genes_all)))
    } else {
...
metabolic_matrix = function(path_names, list_parsed_paths, organism_code,
    ...
    if (is.matrix(enzymeTable) & is.matrix(reactionTable) & is.matrix(koTable)) {
        ...
        reactions_genes = mapply(link_reaction_gene, enzymes, genes, ko, MoreArgs = RT,
                                 SIMPLIFY = FALSE)
        ...
        all_edges_RG = lapply(split(metabolic_table,row(metabolic_table)), build_RG_edge,
                              reactionko = reactionko,reactiongene = reactiongene,
